...
import { parseMajor, parseMajorMinor, } from '../upgrade.utils.js';
const JEST_V30 = '^30.0.0';
const TYPES_JEST_V30 = '^30.0.0';
const TS_JEST_FOR_JEST_30 = '^29.4.0';
export function migrateTestRunner(report) {
    return (tree) => {
        ...
        if (typesJest && typesJestMajor !== null && typesJestMajor < 30) {
            ...
            report.change(`Updated "@types/jest" from "${typesJest.version}" to "${TYPES_JEST_V30}"`);
        }
        ...
        if (tsJest &&
            ...
            report.change(`Updated "ts-jest" from "${tsJest.version}" to "${TS_JEST_FOR_JEST_30}" (first release supporting Jest 30)`);
        }
        report.warn('Jest: the NestJS 12 packages are ESM-only and Jest can only require() ESM modules on Node.js v24.9+ ' +
