...
import { parseMajor, parseMajorMinor, readPackageJson, } from '../upgrade.utils.js';
const NEST_V12 = '^12.0.0';
const GRAPHQL_V14 = '^14.0.0';
export const NEST_V12_PACKAGES = {
...
export function updateNestDependencies(options, report) {
    return (tree) => {
        const updated = [];
        ...
        if (unknown.length > 0) {
            report.warn(`The following @nestjs packages were left untouched because their v12-compatible release is not known to this schematic: ${unknown.join(', ')}. ` +
        ...
        if (typescript && typescriptMajor !== null && typescriptMajor < 6) {
            ...
            report.change(`Updated "typescript" from "${typescript.version}" to "${TYPESCRIPT_V6}" (required by @nestjs/cli and @nestjs/schematics v12)`);
            report.warn('TypeScript 6 deprecates several legacy "tsconfig.json" options (e.g. "baseUrl", "esModuleInterop" defaults changed). Run "npx tsc --noEmit" after installing and adjust your configuration if needed.');
        }
        updateNodeEngine(tree, report);
        return tree;
    };
}
function updateNodeEngine(tree, report) {
    ...
    new JSONFile(tree, '/package.json').modify(['engines', 'node'], MIN_NODE_ENGINE);
    report.change(`Updated "engines.node" from "${current}" to "${MIN_NODE_ENGINE}" (NestJS 12 minimum)`);
}
function findUnknownNestPackages(tree) {
...
export function installDependencies(options, report) {
    return (tree, context) => {
        if (options.skipInstall) {
            report.action("Dependencies were not installed (--skip-install). Run your package manager's install command to finish the upgrade.");
            return tree;
        }
