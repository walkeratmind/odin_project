...
import { applyEdits, collectSourceFiles, getSourceRoots, parseSource, quoteOf, removePackageJsonDependency, } from '../upgrade.utils.js';
const NATS_TRANSPORT_PACKAGE = '@nats-io/transport-node';
const NATS_CORE_PACKAGE = '@nats-io/nats-core';
const NATS_V3 = '^3.0.0';
export function migrateNats(report) {
    return (tree) => {
        ...
        for (const path of collectSourceFiles(tree, getSourceRoots(tree))) {
            ...
            if (edits.length > 0) {
                ...
                report.change(`${path}: rewrote "nats" imports to "${NATS_CORE_PACKAGE}"`);
            }
        ...
        if (!getPackageJsonDependency(tree, NATS_TRANSPORT_PACKAGE)) {
            ...
            report.change(`Added "${NATS_TRANSPORT_PACKAGE}@${NATS_V3}" (NATS v3 driver used by @nestjs/microservices v12)`);
        }
        if (importsRewritten &&
        ...
        if (importsRewritten) {
            report.warn(`Review the symbols you import from "${NATS_CORE_PACKAGE}": NATS v3 dropped the "StringCodec"/"JSONCodec" helpers and reorganised connection types. "headers()" is still available there.`);
        }
        if (customDeserializer) {
            report.warn('NATS transport: Nest 12 serializes NATS packets as JSON strings and custom deserializers now receive the full NATS message instead of a raw "Uint8Array" — read the payload with "msg.json()" instead of decoding bytes manually.');
        }
        else {
            report.note('NATS transport: Nest 12 serializes packets as JSON strings; if you add custom (de)serializers, they receive the full NATS message ("msg.json()").');
        }
        return tree;
    };
}
