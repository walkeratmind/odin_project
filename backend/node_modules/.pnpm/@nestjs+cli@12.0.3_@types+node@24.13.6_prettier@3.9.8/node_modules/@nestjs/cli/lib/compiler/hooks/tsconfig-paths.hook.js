            ...
            const visitNode = (node) => {
                if (tsBinary.isImportDeclaration(node) ||
                    ...
                    try {
                        ...
                        const result = getNotAliasedPath(sf, matcher, text, esm, extensions, emittedPaths);
                        if (!result) {
                            return node;
                        }
...
function sourceToOutputExtension(compilerOptions, tsBinary) {
    // `jsx: preserve` leaves the syntax in place and emits `.jsx`; every other
    // mode compiles it away.
    const jsxOutput = compilerOptions.jsx === tsBinary.JsxEmit.Preserve ? '.jsx' : '.js';
    return {
        '.mts': '.mjs',
        '.cts': '.cjs',
        '.ts': '.js',
 ...
 */
function matchPath(matcher, text, esm) {
 ...
 */
function toEmittedPath(resolvedPath, extensions, cache) {
    ...
    for (const [source, output] of entries) {
        if (resolvedPath.endsWith(source)) {
            return remember(cache, resolvedPath, resolvedPath.slice(0, -source.length) + output);
        }
    }
...
function getNotAliasedPath(sf, matcher, text, esm = false, extensions = {}, emittedPaths = new Map()) {
