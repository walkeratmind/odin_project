 ...
 */
export function appendJsExtensionIfMissing(importPath) {
 ...
 * map values.
 */
export function rewriteAsyncImportString(target) {
    return target.replace(/import\((['"])((?:\\\1|(?!\1).)*)\1\)/g, (match, quote, specifier) => `import(${quote}${appendJsExtensionIfMissing(specifier)}${quote})`);
}
/**
 ...
 */
export function rewriteImportExpressionForNodeNext(expression, tsBinary) {
    const visit = (node) => {
        if (tsBinary.isCallExpression(node) &&
            node.expression.kind === tsBinary.SyntaxKind.ImportKeyword &&
            node.arguments.length > 0 &&
            tsBinary.isStringLiteralLike(node.arguments[0])) {
            const original = node.arguments[0].text;
            ...
            if (rewritten !== original) {
                ...
                return tsBinary.factory.updateCallExpression(node, node.expression, node.typeArguments, updatedArgs);
            }
        }
        return tsBinary.visitEachChild(node, visit, undefined);
    };
 ...
 */
export function rewriteCollectedMetadataForNodeNext(metadata, tsBinary) {
...
export class PluginMetadataGenerator {
    ...
    traverseAndPrintMetadata(programRef, visitors, outputDir, filename) {
        ...
        if (requiresExplicitImportExtensions(programRef.getCompilerOptions(), this.tsBinary)) {
