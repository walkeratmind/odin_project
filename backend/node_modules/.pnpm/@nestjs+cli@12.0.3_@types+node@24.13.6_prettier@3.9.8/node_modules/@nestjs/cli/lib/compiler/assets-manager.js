...
import { areOutsidePathsAllowed, assertPathInsideProject, } from './helpers/path-confinement.js';
const ASSET_CHANGE_RESTART_DEBOUNCE_MS = 150;
export class AssetsManager {
    watchAssetsKeyValue = {};
    watchers = [];
    watcherReadyPromises = [];
    /**
    ...
    copyAssets(configuration, appName, outDir, watchAssetsMode, onSuccess, rootDir) {
        const assets = getValueOrDefault(configuration, 'compilerOptions.assets', appName) || [];
        const includeLibraryAssets = getValueOrDefault(configuration, 'compilerOptions.includeLibraryAssets', appName) || [];
        const libraryAssets = this.collectLibraryAssets(configuration, includeLibraryAssets, outDir);
        ...
        try {
            ...
            const confinePathsToProject = !areOutsidePathsAllowed(configuration, appName);
            ...
            for (const item of allFilesToCopy) {
                ...
                const itemSourceRoot = item._sourceRoot || stripRoot;
                const option = {
                    action: 'change',
                    item,
                    path: '',
                ...
                if (isWatchEnabled || item.watchAssets) {
                    ...
                    if (matchedPaths.length === 0) {
                        console.warn(`No files matched the asset pattern "${item.glob}". ` +
                    ...
                    watcher.on('error', (error) => {
                        console.error(`${ERROR_PREFIX} Asset watcher failed for "${item.glob}". ` +
    ...
    collectLibraryAssets(configuration, libraryNames, outDir) {
        ...
        for (const libName of libraryNames) {
            ...
            const libSourceRoot = join(process.cwd(), libProject.sourceRoot || libProject.root || '');
     ...
     */
    safeActionOnFile(option) {
        try {
            this.actionOnFile(option);
        }
        catch (err) {
            console.error(`${ERROR_PREFIX} ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    actionOnFile(option) {
        const { action, item, path, sourceRoot, watchAssetsMode, confinePathsToProject, } = option;
        ...
        let dest = copyPathResolve(path, item.outDir, sourceRoot.split(sep).length);
        ...
        if (resolvePath(dest) === resolvePath(item.outDir)) {
            throw new Error(`Refusing to copy "${path}" to "${item.outDir}": the "include" path resolves outside ` +
