 ...
 */
export function getEffectiveRootDir(explicitRootDir, fileNames, cwd = process.cwd()) {
    if (explicitRootDir) {
        return normalize(isAbsolute(explicitRootDir)
            ? explicitRootDir
    ...
    const absolutePaths = emittedFiles.map((file) => normalize(isAbsolute(file) ? file : resolve(cwd, file)));
