 ...
 */
function expandGlobstarSuffix(pattern) {
 ...
 */
function symlinkDepthBudget(rest) {
 ...
 */
function walk(dir, recursive, budget, depth = 0) {
    let entries;
    try {
 ...
 */
export function globEntriesSync(pattern, options = {}) {
    ...
    const accept = (candidate) => includeMatchers.some((matcher) => minimatch(candidate, matcher, { dot })) &&
    ...
    const recursive = rest.length > 1 || rest.some((segment) => segment.includes('**'));
